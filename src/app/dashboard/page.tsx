...
export default function DashboardPage() {
    ...
    const { gatewayData, loading, isLive, latestLogs, nodeData } = useTelemetry();
    ...
    const handleExport = async () => {
        ...
        const headers = [
            ...
            'Load (kW)', 'Energy (kWh)', 'Reactive (kVARh)', 'PF',
            'L1 (V)', 'L2 (V)', 'L3 (V)', 'L1 (A)', 'L2 (A)', 'L3 (A)',
            'Temp (°C)', 'Vibration (mm/s)', 'CO2 (kg)'
        ];
        ...
        const rows = filteredDevices.map(d => {
            ...
            return [
                ...
                d.phaseVoltages[0].toFixed(1), d.phaseVoltages[1].toFixed(1), d.phaseVoltages[2].toFixed(1),
                d.phaseCurrents[0].toFixed(1), d.phaseCurrents[1].toFixed(1), d.phaseCurrents[2].toFixed(1),
    ...
    return (
        <div className="space-y-6 pb-10 fade-in px-4 md:px-0">
            ...
            {/* TX2 Data Section (Moved from Reports) */}
            <Card className="glass-card md:rounded-[40px] rounded-3xl border-brand-green-light/5">
                ...
                <CardContent className="px-8 pb-8">
                    {gateway.txNodes.filter(d => d.zone === 'Transmitter 2 (Zone-B)').length > 0 ? (
                        <div className="overflow-x-auto">
                            <Table>
                                ...
                                <TableBody>
                                    {gateway.txNodes.filter(d => d.zone === 'Transmitter 2 (Zone-B)').map(device => (
            ...
            {/* Export Section (Moved from Reports) */}
